	...
	for {
		...
		for platform, tasks := range platformTask {
			...
			if len(nullTaskIds) > 0 {
				...
				if err != nil {
					logging.LogError(ctx, fmt.Sprintf("Fix null task_id task error: %v", err))
				} else {
					logging.LogInfo(ctx, fmt.Sprintf("Fix null task_id task success: %v", nullTaskIds))
				}
			}
...
func UpdateSunoTaskAll(ctx context.Context, taskChannelM map[int][]string, taskM map[string]*model.Task) error {
	for channelId, taskIds := range taskChannelM {
		...
		if err != nil {
			logging.LogError(ctx, fmt.Sprintf("渠道 #%d 更新异步任务失败: %d", channelId, err.Error()))
		}
	}
	return nil
}
...
func updateSunoTaskAll(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	if err != nil {
		...
		if err != nil {
			logging.SysError(fmt.Sprintf("UpdateMidjourneyTask error2: %v", err))
		}
		return err
	}
	...
	if err != nil {
		logging.SysError(fmt.Sprintf("Get Task Do req error: %v", err))
		return err
	}
	...
	if err != nil {
		logging.SysError(fmt.Sprintf("Get Task parse body error: %v", err))
		return err
	}
	...
	for _, responseItem := range responseItems.Data {
		...
		if responseItem.FailReason != "" || task.Status == model.TaskStatusFailure {
			logging.LogInfo(ctx, task.TaskID+" 构建失败，"+task.FailReason)
			task.Progress = "100%"
			//err = model.CacheUpdateUserQuota(task.UserId) ?
			if err != nil {
				logging.LogError(ctx, "error update user quota cache: "+err.Error())
			} else {
				quota := task.Quota
				if quota != 0 {
		...
		err = task.Update()
		if err != nil {
			logging.SysError("UpdateMidjourneyTask task error: " + err.Error())
		}
	}
	return nil
}
